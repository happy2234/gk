...
export const metadata: Metadata = {
  ...
  description: "Get in touch — email, GitHub, LinkedIn, X, Google Scholar.",
};
...
// Map social link id → icon component
function IconForId({ id, size = 18 }: { id: string; size?: number }) {
  switch (id) {
    case "email":    return <Mail size={size} strokeWidth={1.75} aria-hidden="true" />;
    case "github":   return <GithubIcon size={size} />;
    case "linkedin": return <LinkedinIcon size={size} />;
    case "scholar":  return <ScholarIcon size={size} />;
    case "x":        return <XIcon size={size} />;
    default:         return <BookOpen size={size} strokeWidth={1.75} aria-hidden="true" />;
  }
}
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16 animate-fade-in">
      ...
      <div className="flex flex-col max-w-sm">
        {site.socialLinks.map(({ id, label, value, href, external }) => (
          <a
            key={id}
            id={`contact-${id}`}
            href={href}
